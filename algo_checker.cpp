	...
	// Data Structure
		...
		"lagrange":
		{
			...
			[
				...
				"    {",
				"        if (!i) prf[i] = x%mod;",
				"        else prf[i] = (prf[i-1]*((x-i)%mod))%mod;",
				"    }",
				"    for (ll i = n; i >= 0; i--)",
				"    {",
				"        if (i == n) suf[i] = (x-i)%mod;",
				"        else suf[i] = (suf[i+1]*((x-i)%mod))%mod;",
				"    }",
	...
	// Tree
		...
		"lca":
		{
			...
			[
				...
				"    for (ll j = 1; j <= 19; j++)",
				"    {",
				"         for (ll i = 1; i <= n; i++) up[i][j] = up[up[i][j-1]][j-1];",
				"    }",
				"}",
				"ll lca(ll a, ll b)",
				"{",
	...
	// DP
		...
		"FFT2":
		{
			...
			[
				...
				"    init()",
				"    {",
				"        bt.resize(n); pw.resize(n); invpw.resize(n);",
				"        for (ll i = 1, j = inc(0); i < n; i++, j = inc(j)) bt[i] = j;",
				"        pw[0] = invpw[0] = 1;",
				"        double alpha = 2*pi/n;",
				"        cd wl(cosl(alpha),sinl(alpha));",
				"        for (ll i = 1; i < n; i++) {pw[i] = pw[i-1]*wl; invpw[i] = cd(pw[i].real(),-pw[i].imag());}",
				"    }",
				"} init;",
				"void fft(vector<cd>& a, bool invert)",
				"{",
				"    ll t = 31-__builtin_clz(init.n/a.size());",
				"    for (ll i = 0; i < bt.size(); i++) if (i < (bt[i]>>t)) swap(a[i],a[bt[i]>>t]);",
				"    vector<cd>& wpows = (invert ? invpw : pw);",
				"    for (ll l = 2; l <= a.size(); l <<= 1)",
				"    {",
				"        ll jmp = init.n / l;",
				"        for (ll i = 0; i < a.size(); i += l)",
				"        {",
				"            for (ll j = 0; j < l/2; j++)",
				"            {",
				"                cd u = a[i+j], v = a[i+j+l/2] * wpows[j * jmp];",
				"                a[i+j] = u+v; a[i+j+l/2] = u-v;",
				"            }",
				"        }",
				"    } ",
				"    if (invert) for (ll i = 0; i < a.size(); i++) a[i] /= a.size();",
				"}",
				"void multiply(vector<cd>& a, vector<cd> b)",
				"{",
				"    ll n = 1<<(32-__builtin_clz(a.size()+b.size()-2));",
				"    a.resize(n); b.resize(n); fft(a,0); fft(b,0);",
				"    for (ll i = 0; i < n; i++) a[i] *= b[i];",
				"    fft(a,1);",
				"    for (ll i = 0; i < n; i++) a[i] = abs(a[i].real()) > 1e-3;",
				"    while (a.size() && a.back().real() == 0) a.pop_back(); ",
				"} // input vector is complex, when getting value, get a[i].real()",
				"void square(vector<cd>& a)",
				"{",
				"    ll n = 1<<(32-__builtin_clz((a.size()<<1)-2));",
				"    a.resize(n); fft(a,0);",
				"    for (ll i = 0; i < a.size(); i++) a[i] *= a[i];",
				"    fft(a,1);",
				"    for (ll i = 0; i < a.size(); i++) a[i] = abs(a[i].real()) > 1e-3;",
				"    while (a.size() && a.back().real() == 0) a.pop_back();",
				"}"			
			]
		}
